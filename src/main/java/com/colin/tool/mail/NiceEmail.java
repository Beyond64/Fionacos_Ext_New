...
public class NiceEmail {
     ...
     * @return
     */
    public static Properties SMTP_FIONACOS() {
        Properties props = defaultConfig();
        props.put("mail.smtp.host", "mail.fionacos.com");
        return props;
    }
    ...
    public static NiceEmail inUse(Class c) throws InvocationTargetException, IllegalAccessException {
        ...
        if (type.equals("SMTP_163")){
            config(NiceEmail.SMTP_163(),username,password);
        }
        if (type.equals("SMTP_FIONACOS")){
            config(NiceEmail.SMTP_FIONACOS(),username,password);
        }
        return niceEmail;
...
//        });
    ...
    public NiceEmail copyTo(String to) throws MessagingException {
        return addRecipient(to, Message.RecipientType.CC);
    }
